<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1" import="com.cs336.pkg.*"%>
<%@ page import="java.io.*,java.util.*,java.sql.*"%>
<%@ page import="javax.servlet.http.*,javax.servlet.*" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Filtered Flights</title>
</head>
<body>
<h2>
	Flights filtered by
	"<% out.print(request.getParameter("trip_type"));%>"
	going from
	"<% out.print(request.getParameter("dep_airport"));%>"
	to
	"<% out.print(request.getParameter("arr_airport"));%>".
</h2>
	<%  
		try {
			//Get the database connection
			ApplicationDB db = new ApplicationDB();	
			Connection con = db.getConnection();		

			//Create a SQL statement
			Statement stmt = con.createStatement();

			String tripType = request.getParameter("trip_type");
			String depAirport = request.getParameter("dep_airport");
			String arrAirport = request.getParameter("arr_airport");
			String depDate = request.getParameter("dep_date");
			String depDateStart = request.getParameter("dep_date_start");
			String depDateEnd = request.getParameter("dep_date_end");
			String depDateSingle = request.getParameter("dep_date_single");
			String arrDate = request.getParameter("arr_date");
			String arrDateStart = request.getParameter("arr_date_start");
			String arrDateEnd = request.getParameter("arr_date_end");
			String arrDateSingle = request.getParameter("arr_date_single");		
			String numStops = request.getParameter("num_stops");		
	
			String query = "SELECT * FROM flights ";
			boolean where = false;
			
			if (!depAirport.isEmpty() && depAirport != null) {
				query += "WHERE departure_airport LIKE '%" + depAirport + "%' ";
				where = true;
			}
			
			if (!arrAirport.isEmpty() && arrAirport != null) {
				if (!where)
					query += "WHERE arrival_airport LIKE '%" + arrAirport + "%' ";
				else
				{
					query += "AND arrival_airport LIKE '%" + arrAirport + "%' ";
				}
				where = true;
			}
			
			if (!tripType.equals("any")) {
				if (!where)
					query += "WHERE triptype = '" + tripType + "' ";
				else {
					query += "AND triptype = '" + tripType + "' ";
				}
				where = true;
			}
			
			if (depDate.equals("flex")) {
				if (!where)
					query += "WHERE departure_date BETWEEN '" + depDateStart + "' AND '" + depDateEnd + "' ";
				else {
					query += "AND departure_date BETWEEN '" + depDateStart + "' AND '" + depDateEnd + "' ";
				}
				where = true;
			}
			
			if (depDate.equals("inflex")) {
				if (!where)
					query += "WHERE departure_date = '" + depDateSingle + "' ";
				else {
					query += "AND departure_date = '" + depDateSingle + "' ";
				}
				where = true;
			}
			
			if (arrDate.equals("flex")) {
				if (!where)
					query += "WHERE arrival_date BETWEEN '" + arrDateStart + "' AND '" + arrDateEnd + "' ";
				else {
					query += "AND arrival_date BETWEEN '" + arrDateStart + "' AND '" + arrDateEnd + "' ";
				}
				where = true;
			}
			
			if (arrDate.equals("inflex")) {
				if (!where)
					query += "WHERE arrival_date = '" + arrDateSingle + "' ";
				else {
					query += "AND arrival_date = '" + arrDateSingle + "' ";
				}
				where = true;
			}
			
			if (numStops != null && !numStops.isEmpty()) {
				if (!where)
					query += "WHERE num_stops = '" + numStops + "' ";
				else {
					query += "AND num_stops = '" + numStops + "' ";
				}
			}

			ResultSet result = stmt.executeQuery(query);
			
			out.print("<table>");
			out.print("<tr>");
			
			out.print("<td>");
			out.print("Airline");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Aircraft");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Departure Airport");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Departure Date");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Departure Time");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Arrival Airport");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Arrival Date");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Arrival Time");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Trip Type");
			out.print("</td>");
			
			out.print("<td>");
			out.print("# of Stops");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Economy Price");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Business Price");
			out.print("</td>");
			
			out.print("<td>");
			out.print("First Class Price");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Availability");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Book Economy");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Book Business");
			out.print("</td>");
			
			out.print("<td>");
			out.print("Book First Class");
			out.print("</td>");
			
			out.print("<td>");
			out.print("</tr>");

			
			//parse out the results
			while (result.next()) {
				//Get vars
				String airlineId = result.getString("airline_id");
				String aircraftId = result.getString("aircraft_id");
				String depAirport2 = result.getString("departure_airport"); 
				String depDate2 = result.getString("departure_date");
			    String depTime = result.getString("departure_time");
			    String arrAirport2 = result.getString("arrival_airport");
			    String arrDate2 = result.getString("arrival_date");
			    String arrTime = result.getString("arrival_time");
			    String tripType2 = result.getString("triptype");
				String econPrice = result.getString("econ_price");
				String businessPrice = result.getString("business_price");
				String firstclassPrice = result.getString("firstclass_price");
				String availability = result.getString("availability");
				String firstName = (String) session.getAttribute("firstname");
				String lastName = (String) session.getAttribute("lastname");
				
				out.print("<tr>");
				out.print("<td>");
				out.print(result.getString("airline_id"));
				out.print("</td>");
				out.print("<td>");
				
				out.print(result.getString("aircraft_id"));
				out.print("</td>");
				out.print("<td>");
				
				
				out.print(result.getString("departure_airport"));
				out.print("</td>");
				out.print("<td>");
				
				out.print(result.getString("departure_date"));
				out.print("</td>");
				out.print("<td>");
				
				
				out.print(result.getString("departure_time"));
				out.print("</td>");
				out.print("<td>");
				
				out.print(result.getString("arrival_airport"));
				out.print("</td>");
				out.print("<td>");
				
				out.print(result.getString("arrival_date"));
				out.print("</td>");
				out.print("<td>");
				
				out.print(result.getString("arrival_time"));
				out.print("</td>");
				out.print("<td>");
				
				out.print(result.getString("triptype"));
				out.print("</td>");
				out.print("<td>");

				out.print(result.getString("num_stops"));
				out.print("</td>");
				out.print("<td>");
				
				out.print(result.getString("econ_price"));
				out.print("</td>");
				out.print("<td>");
				
				out.print(result.getString("business_price"));
				out.print("</td>");
				out.print("<td>");
				
				out.print(result.getString("firstclass_price"));
				out.print("</td>");
				out.print("<td>");
				
				if (result.getString("availability").equals("1"))
				{
					out.print("Available");
				}
				else
				{
					out.print("NOT Available");
				}
				out.print("</td>");
				out.print("<td>");
				
				if (result.getString("availability").equals("1"))
				{
					%>
					<form action="CustomerConfirmation.jsp">
			    		<input type="submit" name="book" value="Book Economy">
			    		<input type="hidden" name=airline_id value="<%=airlineId%>">
				    	<input type="hidden" name=aircraft_id value="<%=aircraftId%>">
				    	<input type="hidden" name=departure_airport value="<%=depAirport2%>">
				    	<input type="hidden" name=departure_date value="<%=depDate2%>">
				    	<input type="hidden" name=departure_time value="<%=depTime%>">
				    	<input type="hidden" name=arrival_airport value="<%=arrAirport2%>">
				    	<input type="hidden" name=econ_price value="<%=econPrice%>">
				    	<input type="hidden" name=business_price value="<%=businessPrice%>">
				    	<input type="hidden" name=firstclass_price value="<%=firstclassPrice%>">
				    	<input type="hidden" name=first_name value="<%=firstName%>">
				    	<input type="hidden" name=last_name value="<%=lastName%>">
			    	</form>
			    	<%
					out.print("</td>");
					out.print("<td>");
					
					%>
					<form action="CustomerConfirmation.jsp">
			    		<input type="submit" name="book" value="Book Business">
			    		<input type="hidden" name=airline_id value="<%=airlineId%>">
				    	<input type="hidden" name=aircraft_id value="<%=aircraftId%>">
				    	<input type="hidden" name=departure_airport value="<%=depAirport2%>">
				    	<input type="hidden" name=departure_date value="<%=depDate2%>">
				    	<input type="hidden" name=departure_time value="<%=depTime%>">
				    	<input type="hidden" name=arrival_airport value="<%=arrAirport2%>">
				    	<input type="hidden" name=economy_price value="<%=econPrice%>">
				    	<input type="hidden" name=business_price value="<%=businessPrice%>">
				    	<input type="hidden" name=firstclass_price value="<%=firstclassPrice%>">
				    	<input type="hidden" name=first_name value="<%=firstName%>">
				    	<input type="hidden" name=last_name value="<%=lastName%>">
			    	</form>
			    	<%
					out.print("</td>");
					out.print("<td>");
				
					%>
					<form action="CustomerConfirmation.jsp">
			    		<input type="submit" name="book" value="Book First Class">
			    		<input type="hidden" name=airline_id value="<%=airlineId%>">
				    	<input type="hidden" name=aircraft_id value="<%=aircraftId%>">
				    	<input type="hidden" name=departure_airport value="<%=depAirport2%>">
				    	<input type="hidden" name=departure_date value="<%=depDate2%>">
				    	<input type="hidden" name=departure_time value="<%=depTime%>">
				    	<input type="hidden" name=arrival_airport value="<%=arrAirport2%>">
				    	<input type="hidden" name=economy_price value="<%=econPrice%>">
				    	<input type="hidden" name=business_price value="<%=businessPrice%>">
				    	<input type="hidden" name=firstclass_price value="<%=firstclassPrice%>">
				    	<input type="hidden" name=first_name value="<%=firstName%>">
				    	<input type="hidden" name=last_name value="<%=lastName%>">
			    	</form>
			    	<%
					out.print("</td>");
					out.print("<td>");
				}
				else
				{
					%>
					<form action="CustWaitingList.jsp">
			    		<input type="hidden" name=airline_id value="<%=airlineId%>">
				    	<input type="hidden" name=aircraft_id value="<%=aircraftId%>">
				    	<input type="hidden" name=departure_airport value="<%=depAirport2%>">
				    	<input type="hidden" name=departure_date value="<%=depDate2%>">
				    	<input type="hidden" name=departure_time value="<%=depTime%>">
				    	<input type="hidden" name=arrival_airport value="<%=arrAirport2%>">
				    	<input type="hidden" name=fare value="<%=econPrice%>">
				    	<input type="hidden" name=class value="<%="Economy"%>">
				    	<input type="hidden" name=triptype value="<%=tripType2%>">
				    	<input type="hidden" name=first_name value="<%=firstName%>">
				    	<input type="hidden" name=last_name value="<%=lastName%>">
			    		<input type="submit" name="economy" value="Go on Waiting List for Economy">
			    	</form>
			    	<%
					out.print("</td>");
					out.print("<td>");
			    				
					%>
					<form action="CustWaitingList.jsp">
			    		<input type="hidden" name=airline_id value="<%=airlineId%>">
				    	<input type="hidden" name=aircraft_id value="<%=aircraftId%>">
				    	<input type="hidden" name=departure_airport value="<%=depAirport2%>">
				    	<input type="hidden" name=departure_date value="<%=depDate2%>">
				    	<input type="hidden" name=departure_time value="<%=depTime%>">
				    	<input type="hidden" name=arrival_airport value="<%=arrAirport2%>">
				    	<input type="hidden" name=fare value="<%=businessPrice%>">
				    	<input type="hidden" name=triptype value="<%=tripType2%>">
				    	<input type="hidden" name=first_name value="<%=firstName%>">
				    	<input type="hidden" name=class value="<%="Business"%>">
				    	<input type="hidden" name=last_name value="<%=lastName%>">
			    		<input type="submit" name="business" value="Go on Waiting List for Business">
			    	</form>
			    	<%
					out.print("</td>");
					out.print("<td>");
			    				
					%>
					<form action="CustWaitingList.jsp">
			    		<input type="hidden" name=airline_id value="<%=airlineId%>">
				    	<input type="hidden" name=aircraft_id value="<%=aircraftId%>">
				    	<input type="hidden" name=departure_airport value="<%=depAirport2%>">
				    	<input type="hidden" name=departure_date value="<%=depDate2%>">
				    	<input type="hidden" name=departure_time value="<%=depTime%>">
				    	<input type="hidden" name=arrival_airport value="<%=arrAirport2%>">
				    	<input type="hidden" name=fare value="<%=firstclassPrice%>">
				    	<input type="hidden" name=triptype value="<%=tripType2%>">
				    	<input type="hidden" name=first_name value="<%=firstName%>">
				    	<input type="hidden" name=last_name value="<%=lastName%>">
				    	<input type="hidden" name=class value="<%="First Class"%>">
			    		<input type="submit" name="firstclass" value="Go on Waiting List for First Class">
			    	</form>
			    	<%
					out.print("</td>");
					out.print("<td>");
				}

				out.print("</td>");
				out.print("</tr>");

			}
			out.print("</table>");

			//close the connection.
			db.closeConnection(con);
		} catch (Exception e) {
			out.print(e);
		}
	%>
			<p><a href="CustLoginSuccess.jsp">Go to main menu.</a></p>
</body>
</html>